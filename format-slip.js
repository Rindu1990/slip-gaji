...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById('uploadGitHub').addEventListener('click', async () => {
    ...
    pdf.setFontSize(9);
    const pendapatan = [
      ['Gaji Pokok', +data.get('gaji_pokok')], ['Tunjangan Jabatan', +data.get('tunjangan_jabatan')],
      ['Lembur', +data.get('lembur')], ['Uang Makan', +data.get('uang_makan')]
    ];
    ...
    const potongan = [
      ['Potongan PPh 21', +data.get('pph21')], ['Hutang', +data.get('hutang')]
    ];
    ...
    reader.onloadend = async function () {
      console.log("Proses upload ke GitHub dimulai...");
...
Buka di browser: ${rawURL}`);
        setTimeout(() => {
          document.getElementById('qrCanvas').hidden = false;
